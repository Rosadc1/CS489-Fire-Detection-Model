...
type ProcessingMessage = "classifying" | "localizing" | "complete" | null;
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="relative w-full rounded-xl overflow-hidden shadow-2xl bg-gray-100 ring-4 ring-orange-100">
                <img
                  ref={imageRef}
                  src={selectedImage}
                  alt="Fire detection result with original uploaded image"
                  className="w-full h-auto"
                  onLoad={() => setImageLoaded(true)}
                />
                
                {imageLoaded && imageRef.current && result.localization.detectedObjects.map((obj, idx) => {
                  const imgElement = document.querySelector('img[alt="Fire detection result with original uploaded image"]') as HTMLImageElement;
                  const img = imageRef.current!;
                  
                  // dimensions for displayed image
                  const imgNaturalWidth = imgElement.naturalWidth;
                  const imgNaturalHeight = imgElement.naturalHeight;
                  const imgDisplayedWidth = img.width;
                  const imgDisplayedHeight = img.height;

                  const scaleX = imgDisplayedWidth / imgNaturalWidth;
                  const scaleY = imgDisplayedHeight / imgNaturalHeight;

                  const boxLeft = obj.box.x1 * scaleX;
                  const boxTop = obj.box.y1 * scaleY;
                  const boxWidth = (obj.box.x2 - obj.box.x1) * scaleX;
                  const boxHeight = (obj.box.y2 - obj.box.y1) * scaleY;
                  
                  return (
                    <div
                      key={idx}
                      className="absolute border-4 rounded-lg shadow-2xl"
                      style={{
                        left: `${boxLeft}px`,
                        top: `${boxTop}px`,
                        width: `${boxWidth}px`,
                        height: `${boxHeight}px`,
                        borderColor: obj.name === 'fire' ? '#FAE80C' : '#6b7280',
                    ...
                    >
                      <div 
                        className="absolute -top-8 left-0 px-3 py-1 rounded-lg shadow-lg text-sm font-semibold text-white"
                        style={{
                      ...
                      >
                        {obj.name === 'fire' ? (
                          <><Flame className="w-4 h-4 inline mr-1" /> Fire {(obj.confidence * 100).toFixed(0)}% </>
                        ) : (
                          <><Cloud className="w-4 h-4 inline mr-1" /> Smoke {(obj.confidence * 100).toFixed(0)}% </>
                        )}
                      </div>
                    </div>
                  );
                    ...
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-orange-800">
                          The detected fire and smoke locations have been marked in the image above.
                        </p>
                      </div>
                    </div>
